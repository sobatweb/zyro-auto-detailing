...
// Sponsor logo
import spo1 from "../assets/logo/logo-s/1.webp";
import spo2 from "../assets/logo/logo-s/2.webp";
import spo3 from "../assets/logo/logo-s/3.webp";
import spo4 from "../assets/logo/logo-s/4.webp";
            ...
            >
              {isVideo(item.thumbnail) ? (
                <video 
                  ...
                  aria-label={`${item.type} hasil kerja Zyro Autodetailing - ${item.label}`}
                />
              ) : (
                <img 
                  src={item.thumbnail} 
                  alt={`${item.type} hasil kerja Zyro Autodetailing - ${item.label}`}
                  loading="lazy"
                  className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                />
              )}
          ...
          <div className="flex flex-wrap justify-center items-center gap-10 md:gap-20 transition-opacity duration-500">
            {sponsors.map((logo, index) => (
              <img 
                key={index} 
                src={logo} 
                alt={`Official partner Zyro Autodetailing ${index + 1}`}
                loading="lazy"
